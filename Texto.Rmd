...
#
# Área livre
Av <- rep(seq(.2, .7, .05), each = a)
#
# Equação de segundo nível
#
beta_zero <- 2000
beta_zeroj <- beta_zero + 3000*Av + rnorm(b*a, mean = 0, sd = 50)
#
# Equação mista
#
...
```{r}
mundlak <- lmer(ValorUnitario ~ Area_dm + Area_gm + AreaVerde + (1|Bairro), data = amostra)
s2 <- summary(fit_lmer2)
re2 <- ranef(fit_lmer2)
fe2 <- fixef(fit_lmer2)
pr2 <- profile(fit_lmer2)
```
...
```{r}
lote_padrao_A <- data.frame(Area = 400, Area_center = 0, AreaVerde = 0.20, Bairro = "A")
lote_padrao_B <- data.frame(Area = 400, Area_center = 0, AreaVerde = 0.25, Bairro = "B")
lote_padrao_C <- data.frame(Area = 400, Area_center = 0, AreaVerde = 0.30, Bairro = "C")
lote_padrao_D <- data.frame(Area = 400, Area_center = 0, AreaVerde = 0.35, Bairro = "D")
lote_padrao_E <- data.frame(Area = 400, Area_center = 0, AreaVerde = 0.40, Bairro = "E")
lote_padrao_F <- data.frame(Area = 400, Area_center = 0, AreaVerde = 0.45, Bairro = "F")
lote_padrao_G <- data.frame(Area = 400, Area_center = 0, AreaVerde = 0.50, Bairro = "G")
lote_padrao_H <- data.frame(Area = 400, Area_center = 0, AreaVerde = 0.55, Bairro = "H")
lote_padrao_I <- data.frame(Area = 400, Area_center = 0, AreaVerde = 0.60, Bairro = "I")
lote_padrao_J <- data.frame(Area = 400, Area_center = 0, AreaVerde = 0.65, Bairro = "J")
lote_padrao_K <- data.frame(Area = 400, Area_center = 0, AreaVerde = 0.70, Bairro = "K")
```

| Bairro | Modelo de efeitos fixos                        | Modelos de efeitos mistos com variável de segundo nível |
|:------:|:----------------------------------------------:|:-------------------------------------------------------:|
| A      | `r brf(predict(fit, newdata = lote_padrao_A))` | `r brf(predict(fit_lmer2, newdata = lote_padrao_A))`    |              
| B      | `r brf(predict(fit, newdata = lote_padrao_B))` | `r brf(predict(fit_lmer2, newdata = lote_padrao_B))`    |
| C      | `r brf(predict(fit, newdata = lote_padrao_C))` | `r brf(predict(fit_lmer2, newdata = lote_padrao_C))`    |
| D      | `r brf(predict(fit, newdata = lote_padrao_D))` | `r brf(predict(fit_lmer2, newdata = lote_padrao_D))`    |
| E      | `r brf(predict(fit, newdata = lote_padrao_E))` | `r brf(predict(fit_lmer2, newdata = lote_padrao_E))`    |
| F      | `r brf(predict(fit, newdata = lote_padrao_F))` | `r brf(predict(fit_lmer2, newdata = lote_padrao_F))`    |
| G      | `r brf(predict(fit, newdata = lote_padrao_G))` | `r brf(predict(fit_lmer2, newdata = lote_padrao_G))`    |
| H      | `r brf(predict(fit, newdata = lote_padrao_H))` | `r brf(predictInterval(fit_lmer2, newdata = lote_padrao_H, seed = seed, which = 'all')$fit[1])`    |
| I      | `r brf(predict(fit, newdata = lote_padrao_I))` | `r brf(predict(fit_lmer2, newdata = lote_padrao_I))`    |
| J      | `r brf(predict(fit, newdata = lote_padrao_J))` | `r brf(predict(fit_lmer2, newdata = lote_padrao_J))`    |
| K      | `r brf(predict(fit, newdata = lote_padrao_K))` | `r brf(predict(fit_lmer2, newdata = lote_padrao_K))`    |
